...
if not os.path.exists(repo_dir):
    subprocess.run([
        "git",
        "clone",
        "https://github.com/nasa-nccs-hpda/satvision-toa"
    ])
else:
    subprocess.run(["git", "-C", repo_dir, "pull"])

sys.path.append('satvision-toa')
from satvision_toa.utils import load_config  # noqa: E402
from satvision_toa.models.mim import build_mim_model  # noqa: E402
...
class DummyCloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = torch.randint(
            0, self.num_classes, self.mask_dims, dtype=torch.long)
...
class SatVisionUNet(nn.Module):
    def __init__(
        ...
        self.upconv4 = nn.Sequential(
            nn.Conv2d(4096, 2048, 3, padding=1), nn.ReLU(),
            nn.Conv2d(2048, 1024, 3, padding=1))
        self.upconv3 = nn.Sequential(
            nn.Conv2d(1024 + 1024, 1024, 3, padding=1), nn.ReLU(),
            nn.Conv2d(1024, 512, 3, padding=1))
        self.upconv2 = nn.Sequential(
            nn.Conv2d(512 + 512, 512, 3, padding=1), nn.ReLU(),
    ...
    def forward(self, x, mask=None):
        ...
        enc_features = self.encoder.extra_features(x)
