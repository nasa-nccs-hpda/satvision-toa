...
class ConvertABIToReflectanceBT(object):
    ...
    def __call__(self, img):
        ...
        img[:, :, self.emissive_indices] = _ir_calibrate(
...
class ThreeDCloudPTL(pl.LightningModule):
    def __init__(self, model, data_path, label_key, lr=1e-3, batch_size=64):
        ...
        self.trainset = AbiToa3DCloudDataset(
            data_paths=data_path, label_key=label_key, split='train',
            transform=self.transform)
        self.validset = AbiToa3DCloudDataset(
            data_paths=data_path, label_key=label_key, split='valid',
            transform=self.transform)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_loss', self.train_loss_avg, on_step=True,
                 on_epoch=True, prog_bar=True)
        self.log('train_iou', self.train_iou_avg, on_step=True,
                 on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            'val_loss', self.val_loss_avg, on_step=True, on_epoch=True,
            prog_bar=True, sync_dist=True)
        self.log(
            'val_iou', self.val_iou_avg, on_step=True, on_epoch=True,
            prog_bar=True, sync_dist=True)
...
def run_evaluation_and_boxplot(model, dataloader, tag, device='cuda',
                               checkpoint_name='abi_model'):
    """
    ...
    iou_scores = []
    bce_losses = []
    binaryAccuracyMetric = BinaryAccuracy(threshold=0.5).to(device)
    ...
    with torch.no_grad():
        for inputs, targets in dataloader:
            ...
            for i in range(preds_binary.shape[0]):
                iou = calculate_iou(
                    preds_binary[i].cpu().numpy(), targets[i].cpu().numpy())
    ...
    ax[0].text(
        0.2, 0.9, f'mIoU: {mean_iou:.4f}\nacc: {binaryAccuracy:.4f}',
        horizontalalignment='center', verticalalignment='center',
        transform=ax[0].transAxes, fontsize=12, bbox=dict(
...
def save_roc_curve(model, dataloader, device='cuda',
                   checkpoint_name='abi_model'):
    """
...
def run_prediction_and_plot(model, dataloader, tag, device='cuda',
                            num_predictions=5, checkpoint_name='abi_model'):
    """
    Run prediction on a single batch from the dataloader and plot
    the input image and predicted output.
    """
    model.eval()
    model.to(device)
    ...
    # Switch dimensions 2 and 3 (91x40 to 40x91)
    preds_binary = np.rot90(preds_binary, k=1, axes=(2, 3))
    ...
    fig, axes = plt.subplots(
        num_predictions, 4, figsize=(20, 6 * num_predictions), dpi=300)
    ...
    for i in range(num_predictions):
        ...
        # 100 points along the x-axis, and corresponding y vals
        x_values = np.linspace(128/2+9, 128/2-9, 100)
        y_values = np.linspace(0, 128-1, 100)

        # Viridis colormap, gradient mapped to transect x/y vals
        ...
        for j in range(len(x_values)-1):
            axes[i, 0].plot(
                ...
                [y_values[j], y_values[j+1]], color=colors[j], lw=2)

        axes[i, 0].set_title('ABI image chip (channels [1, 2, 3])')
        axes[i, 0].axis('on')

        cmap = cm.get_cmap('cool')
        # Number of points along the x-axis (image width)
        num_points = targets[i].shape[1]
        # X-coordinates along the bottom
        x_values = np.linspace(0, num_points-1, num_points)
        ...
        axes[i, 1].matshow(preds_binary[i, 0], cmap='viridis')
        ...
        for j in range(len(x_values)-1):
            axes[i, 1].plot(
                [x_values[j], x_values[j+1]],
                [y_value, y_value], color=colors[j], lw=7)

        # Display the predicted binary mask
        save_dict['truths'].append(targets[i])
        axes[i, 2].matshow(targets[i], cmap='viridis')
        axes[i, 2].set_title('CloudSat/CALIPSO "Truth"')
        axes[i, 2].axis('on')
        ...
        axes[i, 3].matshow(diffs[i], cmap='bwr', vmin=-1, vmax=1)
        axes[i, 3].set_title('Prediction vs Truth Difference')
        axes[i, 3].axis('on')
...
def main(args, config):
    DATA_PATH = os.path.join(
        '/explore/nobackup/people/jli30/workspace',
        '/cat-dog/chips/abiChipsPick')
    # IN_CHANNELS: int = 14
    OUTPUT_CHANNELS: int = 1

    pretrained_model = load_pretrained_model(config)
    model = FCN(
        swin_encoder=pretrained_model.encoder,
    ...
    if args.inference:
        ...
        if not os.path.exists(output_path):
            # Use this if performing inference from a PTL checkpoint that has
            # the zero_* associated files

            # Use this if performing inference from a torch checkpoint
            # w/out the zero_* associated files
            checkpoint = torch.load(args.checkpoint)
            checkpoint_model = checkpoint['module']
            checkpoint_model = {
                k.replace('model.', ''): v
                for k, v in checkpoint_model.items() if k.startswith('model')}
            model.load_state_dict(checkpoint_model, strict=False)

        else:
            print(f'Using previously made checkpoint file {output_path}')

        ptlCheckpoint = ThreeDCloudPTL(
            model, data_path=[DATA_PATH], label_key='Cloud_mask',
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='number of epochs for patience')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Number of samples to run inference on and plot'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Freeze the parameters of the encoder'
    )
    parser.add_argument(
        '--tag', type=str, default='svtoa-fcn-3dcloud',
        help='tag to use for plot titles'
    )
